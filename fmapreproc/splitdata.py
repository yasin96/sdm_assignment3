...
def split_ids(path):
    ...
    return np.array(train.index).reshape(-1), np.array(test.index).reshape(-1), np.array(validate.index).reshape(-1),
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        root = "D:\\Fakulteti\\SDM\\fma_metadata\\"
    else:
        root = sys.argv[1]
    train_ids, test_ids, validate_ids = split_ids(root + "tracks.csv")
    ...
    split_root = os.path.dirname(__file__) + "\\split\\"
